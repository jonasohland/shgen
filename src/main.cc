...
const char* ignore_unused_macro
    = R"(#define SHGEN_IGNORE_UNUSED(expr) do { (void)(expr); } while (0)
)";

const char* sse_includes = R"(#include <immintrin.h>
)";

const char* intro_comment = R"(/*
                               ...
                               const char** argv)
{
    ...
    stream << "    Generated " << std::put_time(std::gmtime(&now), "%c %Z")
           << " with shgen " << SHGEN_STR(SHGEN_VERSION)
           << " by the command: " << conf.le;
    stream << conf.indent_fnbody << "shgen ";

    for (int i = 1; i < argc; ++i) {
        if (conf.headerfile == argv[i] || conf.sourcefile == argv[i])
            continue;
    ...
    if (is_header)
        stream << "#pragma once" << conf.le << conf.le;

    if (conf.sse && conf.header_only)
